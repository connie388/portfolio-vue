  ...
  <div>
    <div class="flex flex-col">
      <div class="container mx-auto px-5 py-10">
        <div
          class="relative rounded-md border-2 border-gray-300 dark:border-white p-10 grid grid-cols-1 xxxs:grid-cols-2 xxs:grid-cols-3 sm:grid-cols-4 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-7 2xl:grid-cols-8 gap-4"
        >
          <div v-for="skill in selectedList" :key="skill">
            <a :href="`${skill.link}`" target="_blank" class="flex">
              <span
                class="relative rounded-full bg-sky-100 hover:bg-yellow-100 hover:scale-95 dark:bg-white dark:hover:bg-yellow-300 p-2 md:p-4 mb-1"
              >
                <img
